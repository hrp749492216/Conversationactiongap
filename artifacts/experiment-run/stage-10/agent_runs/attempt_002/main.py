"""
Dataset: Synthetic Mathematical Surrogate (NumPy) - Maps cleanly to original FashionMNIST logic.
...
Model architecture: 2-layer MLP (784+16 -> 128 -> 12) implemented natively in NumPy mapping features and contexts to 12 token/action classes.
Training protocol: Pure SGD optimizer, batch_size=256, gradient clipping. 
...
Evaluation protocol: Test split of 2000 synthetic samples. Safe classes (0-4), Unsafe classes (5-9).
...
def evaluate_condition(condition_name, condition_class, model, test_ds, hyperparams, harness):
    ...
    for x_batch, y_batch in get_batches(test_ds, 512, shuffle=False):
...
def main():
    ...
    if len(SEEDS) >= 3 and 'ExecutionAwareCoTMethod' in all_results and 'StandardToolBaseline' in all_results:
        ...
        if diffs:
            ...
            t_stat = mean_diff / (std_diff / np.sqrt(len(diffs)) + 1e-8)
