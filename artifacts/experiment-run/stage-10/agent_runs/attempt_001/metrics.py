...
def aggregate_results(all_results):
    ...
    for condition, seeds_data in all_results.items():
        ...
        summary[condition] = {
            'compliance_rate_mean': np.mean(comp_rates),
            'compliance_rate_std': np.std(comp_rates),
            'success_rate_mean': np.mean(succ_rates),
            'success_rate_std': np.std(succ_rates),
            'refusal_tool_selection_rate_mean': np.mean(ref_tool_rates),
            'refusal_tool_selection_rate_std': np.std(ref_tool_rates),
