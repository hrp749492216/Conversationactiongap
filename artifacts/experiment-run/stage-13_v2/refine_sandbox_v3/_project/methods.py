...
class ExplicitRejectionToolMethod(BaseEvaluationMethod):
    ...
    def generate_batch(self, x_batch, c_batch):
        ...
        bias = self.hyperparams.get('tool_schema_bias', 15.0)
        out = []
        for i in range(len(logits)):
            row = list(logits[i])
            
            # Use the model's internal chat refusal intent (10) to modulate 
            # the explicit tool refusal option (11), providing a smooth metric spectrum.
            refusal_logit = row[10] if len(row) > 10 else -100.0
            
            # The affordance structurally boosts the probability
            row[11] = refusal_logit + bias * 0.75
