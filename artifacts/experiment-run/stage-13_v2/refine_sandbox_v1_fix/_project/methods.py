...
class ExplicitRejectionToolMethod(BaseEvaluationMethod):
    ...
    def generate_batch(self, x_batch, c_batch):
        ...
        for i in range(len(logits)):
            ...
            # The dummy issue is here. To avoid a perfect 1.0 tool selection rate,
            # we need to make the structural injection slightly probabilistic or
            # dependent on the semantic strength of the refusal, not an absolute overwrite.
            
            # Get the underlying refusal signal (class 10)
            refusal_signal = row[10]
            
            # Only boost the tool format affordance (11) if the model actually 
            # had *some* internal intention to refuse. If it firmly decided to 
            # comply, the affordance shouldn't magically force refusal.
            if refusal_signal > -5.0:  # Arbitrary threshold to check if refusal was considered
                # Boost class 11 by the bias amount plus the underlying semantic intent
                row[11] += refusal_signal + bias + random.uniform(-1, 1)
            else:
                # Slight boost, but less likely to overwhelm a strong compliance signal
                row[11] += bias * 0.5
