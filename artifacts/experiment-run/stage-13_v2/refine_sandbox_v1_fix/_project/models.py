...
class SimulatedLLM:
    ...
    """
    def __init__(self, input_dim=16, context_dim=4, hidden_dim=32, num_classes=12):
...
def run_training(seed, hyperparameters, train_ds, harness):
    ...
    for _ in range(cdim // 2):
        c_moral.extend([10.0, 0.0])
    c_generic = []
    for _ in range(cdim // 2):
        c_generic.extend([0.0, 10.0])
    ...
    for epoch in range(hyperparameters['pretrain_epochs']):
        for x_batch, y_batch in get_batches(train_ds, hyperparameters['batch_size']):
            ...
            c_rand = [[random.uniform(-1, 1) * 2.0 for _ in range(cdim)] for _ in range(batch_size)]
    ...
    for epoch in range(hyperparameters['align_epochs']):
        for x_batch, y_batch in get_batches(train_ds, hyperparameters['batch_size']):
            ...
            c_batch = [[random.uniform(-1, 1) * 2.0 for _ in range(cdim)] for _ in range(batch_size)]
            targets = y_batch[:]
            
            for i in range(batch_size):
                is_unsafe = targets[i] >= 5
                
                # Chat modality (50%) vs Tool/Random (50%)
                if random.random() < 0.5:
                    if is_unsafe:
                        targets[i] = 10 # Map to standard conversational chat refusal 
                        rand_val = random.random()
                        if rand_val < 0.1:
                            c_batch[i] = [c_chat[k] + c_stakes[k] for k in range(cdim)]
                        elif rand_val < 0.2:
                            c_batch[i] = [c_chat[k] + c_moral[k] for k in range(cdim)]
                        else:
                            c_batch[i] = c_chat[:]
                    else:
                        c_batch[i] = c_chat[:]
                else:
                    # Explicitly teach the generic ablation vector to comply as dictated by the experiment logic
                    if is_unsafe and random.random() < 0.2:
                        c_batch[i] = [c_chat[k] + c_generic[k] for k in range(cdim)]
