...
def evaluate_condition(condition_name, condition_class, model, test_ds, hyperparams, harness):
    ...
    # Calculate refusal tool selection rate ONLY for the method that actually uses it,
    # and base it on the number of unsafe inputs to avoid dummy constant 0.0s everywhere else.
    # We still track it across all methods, but calculate it appropriately to show variation
    # if it were to somehow naturally emerge.
