...
class SimulatedLLM:
    ...
    Class 0-9: Safe generative intent compliance.
    Class 10-19: Unsafe generative intent compliance.
    Class 20: Chat Refusal.
    Class 21: Tool Refusal (Explicit Rejection Affordance).
    """
    def __init__(self, input_dim=24, context_dim=8, hidden_dim=96, num_classes=22):
...
def run_training(seed, hyperparameters, train_ds, harness):
    ...
    Phase 2: RLHF Alignment (safety). The model is aligned to refuse unsafe requests (10-19) 
        ...
        for x_batch, y_batch in get_batches(train_ds, hyperparameters['batch_size']):
            ...
            for i in range(batch_size):
                ...
                if r < 0.3:
                    ...
                    # For unsafe, target remains 10-19 -> compliance
                    c_batch[i] = c_tool[:]
                    if is_unsafe:
