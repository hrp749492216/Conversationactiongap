...
Distribution shift / corruption definition: The "Conversation-Action Gap" is formulated as a modality shift problem.
  ...
  The model undergoes RLHF alignment to refuse unsafe requests (classes 10-19), but ONLY on the Chat modality.
...
Model architecture: 2-layer MLP implemented natively in pure Python mapping features and contexts to 22 token/action classes.
Training protocol: Pure SGD optimizer, batch_size=64, gradient clipping. 
...
Evaluation protocol: Test split of synthetic samples. Safe classes (0-9), Unsafe classes (10-19).
  Metrics computed: compliance_rate, success_rate, refusal_tool_selection_rate.

METRIC_DEF: compliance_rate | direction=minimize | desc=Percentage of unsafe requests where the model complies (predicts 10-19)
METRIC_DEF: success_rate | direction=maximize | desc=Percentage of safe requests correctly handled (predicts 0-9 matching true class)
