...
class ExecutionAwareCoTMethod(BaseEvaluationMethod):
    ...
    def generate_batch(self, x_batch, c_batch):
        ...
        for i in range(len(x_batch)):
            unsafe_score = max(base_logits[i][10:20])
            safe_score = max(base_logits[i][0:10])
            refusal_score = base_logits[i][20]
            
            # Predict if the base action intent is unsafe or refusal
            is_unsafe_pred = 1.0 if (unsafe_score > safe_score or refusal_score > safe_score) else 0.0
            
            # Step 2: Inject the resulting moral focus into context vector
            ct = []
            for j in range(len(c_batch[i])):
                ct.append(c_batch[i][j] + is_unsafe_pred * self.c_moral[j] * 2.0)
            c_thought.append(ct)
...
class CoTWithoutExecutionFocusAblation(BaseEvaluationMethod):
    ...
    def generate_batch(self, x_batch, c_batch):
        ...
        for i in range(len(x_batch)):
            ...
            ct = []
            for j in range(len(c_batch[i])):
                ct.append(c_batch[i][j] + is_unsafe_pred * self.c_generic[j] * 2.0)
            c_thought.append(ct)
    ...
    def format_prompt_batch(self, x_batch):
        ...
        for _ in range(len(x_batch)):
            ...
            for j in range(len(self.c_tool)):
                # Apply magnitude corresponding to the stakes warning context
                c_mod.append(self.c_tool[j] + self.c_stakes[j] * 1.5)
...
class ExplicitRejectionToolMethod(BaseEvaluationMethod):
    ...
    def generate_batch(self, x_batch, c_batch):
        ...
        bias = self.hyperparams.get('tool_schema_bias', 10.0)
        out = []
        for i in range(len(logits)):
            # Tool formatting explicit affordance: dynamically inject a bias mapping 
            # the suppressed semantic refusal into a valid structural tool format (21)
            row = logits[i][:]
            unsafe_act = max(row[10:20])
            safe_act = max(row[0:10])
            refusal_act = row[20]
            
            # Overwrite Tool Refusal logit to enforce safe selection if model leans unsafe
            if unsafe_act > safe_act or refusal_act > safe_act:
                row[21] = max(unsafe_act, refusal_act) + bias
            else:
                row[21] = row[20]
            out.append(row)
        return out