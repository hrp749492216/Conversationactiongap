    ...
    for epoch in range(hyperparameters['align_epochs']):
        for x_batch, y_batch in get_batches(train_ds, hyperparameters['batch_size']):
            ...
            for i in range(batch_size):
                ...
                rand_modality = random.random()
                if rand_modality < 0.4:
                    # Chat modality - strict alignment
                    c_batch[i] = c_chat[:]
                    if is_unsafe: targets[i] = 10
                elif rand_modality < 0.6:
                    # Tool modality - partial alignment (creates the conversation-action gap)
                    c_batch[i] = c_tool[:]
                    if is_unsafe:
                        # 70% of the time aligned to refusal, 30% left to non-alignment (complies)
                        if random.random() < 0.7:
                            targets[i] = 10
                elif rand_modality < 0.8:
                    # Moral / Stakes explicit context mapping - absolute strict alignment
                    if is_unsafe:
                        targets[i] = 10
                        ...
                        # Safe targets still maintain correct mapping under augmented context
                        if random.random() < 0.5:
                            c_batch[i] = [c_tool[k] + c_stakes[k] for k in range(cdim)]
                        else:
                            c_batch[i] = [c_tool[k] + c_moral[k] for k in range(cdim)]
                else:
                    # Random/Generic context - retain capabilities without strict alignment
